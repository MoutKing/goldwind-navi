 ...
 * 
 */
package com.gw.web.common.multithread;
...
public class AsyncRunnerUtil {
	...
	private static final Log LOGGER = LogFactory.getLog(AsyncRunnerUtil.class);
	private static final ExecutorService EXECUTORSERVICE = Executors.newFixedThreadPool(POOL_SIZE);
	private static final ReentrantLock LOCK = new ReentrantLock();
	private static SortedMap<String, CallableResult> callContainer = Collections
			.synchronizedSortedMap(new TreeMap<String, CallableResult>());;
	...
	public static String[] run(Object targetObject, String[] runnerIds, AsyncRunnerParams[] idParams)
				...
				for (Method method : methods) {
					if (method.isAnnotationPresent(AsyncRunner.class)) {
						...
						for (Annotation ano : annotations) {
							...
							name = method.getName();
							LOGGER.debug(
									"asyncRunner : " + method.getName() + " [id = " + id + ", name = " + name + "]");
